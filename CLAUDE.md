...
| `TaskWatchRepository.listWaitingForTask` | Uncalled from TS — the M2 fire path lives in a **DB trigger** that runs the same `task_ids @> ARRAY[$1] WHERE status='waiting'` query in SQL. The TS half mirrors a live query (and its partial-GIN access path) and is the only place that behaviour is asserted; the trigger has no test of its own. |
| `WorkspaceManager.removeWorkspace` (port + `LocalWorkspaceManager` impl) | The only workspace-deletion primitive in the port, and **nothing calls it** — so `~/.beevibe/workspaces/<agent_id>` leaks on agent archive. That's a missing call site, not a dead method: deleting it means re-writing the primitive when workspace GC lands. |
| `MemoryFactRepository.listByAgentScope` | No production caller, but it is the only non-vector read of `memory_fact` and `api/src/routes/mcp.test.ts` uses it to assert what `save_memory` wrote end-to-end. Removing it means re-implementing the query as raw SQL inside a test. |
