...
| `GET /health/runtime`, `POST /me/onboarding/complete`, `POST /learned-skills/:id/publish` | Mounted routes with no in-repo caller — an endpoint-vs-`lib/api/client.ts` sweep flags them alongside `GET /activity`, and they're the same **product call**, not dead code. `/health/runtime` is documented (`packages/api/README.md`) as the welcome wizard's "verify daemon" step, whose UI hasn't landed. `/me/onboarding/complete` is a manual override of a flip the chat route already does itself (`routes/chat.ts` flips `onboarding_completed_at` after the onboarding turn). `/learned-skills/:id/publish` opens the community-capability PR and is reachable by any API-key holder. |
