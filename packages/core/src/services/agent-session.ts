...
export class AgentSession {
  ...
  async run(input: AgentSessionRunInput): Promise<Session> {
    ...
    const system_prompt_append = [
      // M9: cross-agent constants pinned first so the prompt cache stays
      // hot across sessions for the same agent. Per-agent baseline +
      // briefing's stable core_memory follow; caller-supplied extras
      // come last (least cache-stable).
    ...
    if (result.output) {
      void eventRepo
        ...
        .catch((err) =>
          console.error(
            `[AgentSession] session_event summary append failed for ${sid}:`,
            (err as Error).message,
          ),
        );
    }
