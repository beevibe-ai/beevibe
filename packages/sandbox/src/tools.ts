/**
 * The five sandbox tools the child claude session is allowed to call.
 *
 * Split out of `./mcp-server.ts` so they can be exercised without
 * importing that module — `mcp-server.ts` runs `main()` on import (it is
 * a `bin` entrypoint spawned over stdio), which would bind to a real
 * container and take over the process's stdio.
 *
 * Each handler wraps a primitive from `./docker.ts`, validates its input
 * (the MCP `inputSchema` is advisory — the SDK forwards whatever the
 * model sent), and caps the response so a runaway command can't blow up
 * the child agent's context.
 */
import {
  exec,
  exportArtifact,
...
export function makeTools(sandbox: Sandbox): ToolDef[] {
  return [
    {
      name: "sandbox_exec",
      description:
