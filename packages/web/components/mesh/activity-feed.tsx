...
import { ArrowRight, Check, GitPullRequest, Link as LinkIcon, Loader2, Ban } from "lucide-react";
import { cn } from "@/lib/utils";
import { fixtureMeshAsks, type MeshAsk } from "@/lib/fixtures/mesh";
...
function AskCard({ ask }: { ask: MeshAsk }) {
  ...
  return (
    <Link
      href={`/sessions/${ask.source_session}`}
      className={cn(
        ...
        isLive
          ...
          : isBlocked
            ? "border-status-blocked/40 bg-status-blocked/5 hover:border-ring/40"
    ...
    >
      <div className="flex items-center gap-2 mb-2 text-xs">
        ...
        <ArrowRight
          className={cn(
            "h-3 w-3",
            isLive ? "text-status-running" : isBlocked ? "text-status-blocked" : "text-muted-foreground",
          )}
        />
        <span className="font-mono text-foreground">{ask.target}</span>
        <StatusBadge ask={ask} />
      </div>
      <p className="text-sm leading-relaxed text-foreground/85">{ask.intent}</p>
      {ask.response ? (
        <div className="mt-2 rounded bg-secondary/40 p-2 text-xs leading-relaxed">
          ...
          <span className="text-foreground/85">{ask.response.content}</span>
        </div>
      ) : null}
        ...
        <span>
          ...
          {!ask.source_task_age && ask.source_task_short_id ? (
            <>
              {" "}
              · from{" "}
              <span className="hover:text-foreground transition-colors">{ask.source_task_short_id}</span>
            </>
          ) : null}
        </span>
      </div>
