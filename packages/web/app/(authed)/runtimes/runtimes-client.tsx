...
import { ListGate } from "@/components/load-gate";
      ...
      <div className="pt-8 pb-12 px-6">
        ...
        <div className="max-w-3xl mx-auto">
          <ListGate
            ...
            noun="runtimes"
            query={query}
            // Guarded: `describeError(null)` stringifies to "null", and
            // this is evaluated on every render, not just the error one.
            errorDescription={query.error ? describeError(query.error) : undefined}
            skeleton={
            ...
            isEmpty={(res: RuntimesListResponse) => (res.daemons ?? []).length === 0}
            empty={<NoDaemonsState />}
          >
            {(res) => (
              <div className="space-y-3">
                {(res.daemons ?? []).map((d) => (
