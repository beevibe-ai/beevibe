...
import { makeCodeErrorHandler, requireParam } from "./http-errors.js";
...
/** 500 responder for this router — logs `[repo-runs/<op>]`, answers with the bare code. */
const fail = makeCodeErrorHandler("repo-runs");
...
export function createRepoRunsRouter(deps: RepoRunsRouterDeps): Router {
  ...
  router.post("/:id/cancel", async (req, res) => {
    ...
    try {
      ...
      fail(err, res, "cancel", "cancel_failed");
    }
  });
  ...
  router.get("/:id/artifacts/:file", async (req, res) => {
    ...
    try {
      ...
      fail(err, res, "artifact", "artifact_failed");
    }
  });
